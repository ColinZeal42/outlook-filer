          ...
          try {
            ...
            if (!match) {
              return event.completed({
                allowEvent: false,
                errorMessage: "DEBUG: No match. Subject=\"" + subject + "\", folders: " + folders.map(function(f) { return f.displayName; }).join(", "),
              });
            }
            ...
            event.completed({ allowEvent: false, errorMessage: "DEBUG error: " + e.message });
          }
        });
      } catch(e) {
        event.completed({ allowEvent: false, errorMessage: "DEBUG error: " + e.message });
...
function graphGet(token, url, onSuccess, onError) {
  ...
  xhr.onload = function() {
    if (xhr.status >= 200 && xhr.status < 300) {
      ...
      if (onError) onError(new Error("Graph GET " + xhr.status));
    }
  };
...
function graphPost(token, url, body, onSuccess, onError) {
  ...
  xhr.onload = function() {
    if (xhr.status >= 200 && xhr.status < 300) {
      ...
      if (onError) onError(new Error("Graph POST " + xhr.status));
    }
  };
  xhr.onerror = function() { if (onError) onError(new Error("Network error")); };
